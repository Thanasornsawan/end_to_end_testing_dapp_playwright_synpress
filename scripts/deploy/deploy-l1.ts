...
async function main() {
  console.log("Deploying contracts to L1 (Ethereum)...");
  ...
  try {
    ...
    console.log("Integration service initialized and database synced");
  } catch (error) {
    console.warn("Failed to initialize integration service:", error);
    console.log("Continuing deployment without integration service");
  }
  ...
  // CRITICAL: Add approvals for WETH from deployer to DelegateManager and AutoRebalancer
  ...
  // Save the addresses to the network configuration
  const networkName = 'local'; // For local Ethereum L1
  const addresses = {
